...
import net.blogsv.Adapters.UserAdapter;
...
import net.blogsv.ui.fragement.UserFragment;
...
public class UserActivity extends AppCompatActivity {
    ...
    private void initUser() {
        ...
        if (!image.isEmpty()) {
            Picasso.get().load(image).error(R.drawable.logo_w).placeholder(R.drawable.profile).into(this.image_view_profile_user_activity);
        } else {
            Picasso.get().load(R.drawable.logo_w).error(R.drawable.logo_w).placeholder(R.drawable.profile).into(this.image_view_profile_user_activity);
        }
        PrefManager prf = new PrefManager(getApplicationContext());
            ...
            public void onResponse(Call<ApiResponse> call, Response<ApiResponse> response) {
                ...
                if (response.isSuccessful()) {
                    ...
                    for (int i = 0; i < response.body().getValues().size(); i++) {
                        ...
                        if (response.body().getValues().get(i).getName().equals("status")) {
